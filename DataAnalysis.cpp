...
int main(int argc, char *argv[])
{
  ...
  cout<<"Last edit:   Jan 20 2018, by amorusor"<<endl;
  cout<<"Compiled at: "<< __DATE__ <<", "<< __TIME__<<"."<<endl;
  
  string execname=argv[1];  //Salvo il nome del file che voglio analizzare
  std::string sourcename=execname.substr(0,execname.size()-12);  //tolgo "Reduced.root" alla fine per usare il nome file in seguito

  cout<<"from source file: "<<execname<<endl;
  
  string inputfname="";
  string outfname=Form("%sAnalized.root", sourcename.c_str());
  string badfname="";

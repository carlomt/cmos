...
//	ClassePed(TTree *tree=0);
	ClassePed(TString filename);
	TString nomefile;
   virtual ~ClassePed();
...
#ifdef ClassePed_cxx
ClassePed::ClassePed(TString filename)
{
	nomefile=filename;
...
/*
ClassePed::ClassePed(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
...
Int_t ClassePed::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t ClassePed::LoadTree(Long64_t entry)
{
...
void ClassePed::Init(TTree *tree)
{
...
void ClassePed::Show(Long64_t entry)
{
...
Int_t ClassePed::Cut(Long64_t entry)
{

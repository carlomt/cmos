...
int Riduzione(string fname,double thres, string pedfname, string noisefname, size_t fiducialSideDim=4,  const size_t seedSide=7, const size_t localMaximumCheckSide=3, string outfname="reduced.root",  int FrameNCol=640, int FrameNRow=480, bool MCflag=kFALSE);
...
int main(int argc, char *argv[])
{
	...
	return  Riduzione( inputfname, thres, pedfname, noisefname, fiducialEdge,  seedSide,  localMaximumCheckSide,  outfname, FrameNCol, FrameNRow, MCflag);
	
}


int Riduzione(string fname,double thres, string pedfname, string noisefname, size_t fiducialSideDim,  const size_t seedSide, const size_t localMaximumCheckSide, string outfname, int FrameNCol, int FrameNRow, bool MCflag)
{
	...
	{
		std::cerr<<"WARNING: The pedestal file is (almost) mandatory."<<endl;
		PedFlag=kFALSE;
		//exit(-1);
	}
	else
	...
	// Apro il file che voglio ridurre
	...
	Frame *frameData = new Frame(FrameNCol, FrameNRow);           //Creo l'oggetto frame con i dati
	...
	for (Long64_t jentry=0; jentry<nentries;jentry++)       // CICLO PRINCIPALE SUI FRAME
	{
		...
		if(PedFlag && subtractPed)
		{
			
			frameData->Subtract(*pedestal);
	...
	if(PedFlag)  pedestal->Write("PedestalFrame");                                        //nel file Reduced.root salvo il Frame di piedistallo...
#if 1
